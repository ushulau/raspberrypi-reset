...
const MA_TIME_INTERVAL = 3 * 60 * 1000; // 3 minutes
...
function discoverWemoDevices(deviceIdentifier, fieldName) {
    return new Promise((resolve, reject) => {
        ...
        wemo.discover(function (err, deviceInfo) {
            ...
            if (deviceInfo && deviceInfo[fieldName].toLowerCase().trim() === deviceIdentifier.toLowerCase().trim()) {
                ...
                let wemoClient = wemo.client(deviceInfo);
                ...
                wemoClient.on('error', function (err) {
                ...
                wemoClient.on('binaryState', function (value) {
                    ...
                    client.bulk({
                        body: [
                            ...
                            {ts: new Date().getTime(), timestamp: new Date(), friendlyName: deviceInfo.friendlyName, macAddress: deviceInfo.macAddress, serialNumber: deviceInfo.serialNumber, state: value},
...
let setState = function (val) {
    return new Promise(resolve => {
        if (wemoClient) {
            wemoClient.setBinaryState(val, (err, st) => {
                if (err) {
                    console.error(err);
                    resolve(undefined);
                } else {
...
if (deviceIdentifier) {
    discoverWemoDevices(deviceIdentifier, fieldName).then((client, deviceInfo) => {
        wemoClient = client;
        devInfo = deviceInfo;
